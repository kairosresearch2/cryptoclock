...
def adicionar_id_se_faltar(lista_de_compras):
    for compra in lista_de_compras:
        ...
        if "quantidade_restante" not in compra:
            compra["quantidade_restante"] = compra["quantidade"]
...
def renderizar_cartao_metrica(icone, label, valor_formatado, cor_classe=""):
    """Função reutilizável para desenhar um cartão de métrica com
    'vidro fosco', ícone e indicador néon à esquerda."""
    st.markdown(f"""
    <div class="kc-metric-card {cor_classe}">
        <div class="kc-metric-label">{icone} {label}</div>
        <div class="kc-metric-value {cor_classe}">{valor_formatado}</div>
    </div>
    """, unsafe_allow_html=True)


# ---------------------------------------------------------
# MOTOR FIFO — REGRA DOS 365 DIAS (ARTIGO 10.º DO CÓDIGO DO IRS)
# ---------------------------------------------------------
def obter_lotes_fifo(ativo, quantidade_vender, preco_venda_unitario=None):
    """
    Percorre os lotes de um ativo por ordem cronológica (FIFO — o mais
    antigo primeiro) e determina, para uma venda de `quantidade_vender`
    unidades, que fatia vem de lotes já isentos (>365 dias) e que fatia
    ainda está sujeita a IRS (<=365 dias).

    Se `preco_venda_unitario` for fornecido, calcula também a mais-valia
    (ganho) de cada fatia, separando o ganho isento do ganho tributável.
    """
    hoje = date.today()

    lotes_do_ativo = [
        c for c in st.session_state.compras
        if c["ativo"] == ativo and c.get("quantidade_restante", c["quantidade"]) > 1e-12
    ]
    lotes_ordenados = sorted(lotes_do_ativo, key=lambda c: c["data_compra"])

    restante_a_vender = quantidade_vender
    detalhe_lotes = []
    quantidade_isenta = 0.0
    quantidade_tributavel = 0.0
    ganho_isento = 0.0
    ganho_tributavel = 0.0
    custo_total_vendido = 0.0

    for lote in lotes_ordenados:
        if restante_a_vender <= 1e-12:
            break

        qtd_disponivel = lote.get("quantidade_restante", lote["quantidade"])
        qtd_usada = min(qtd_disponivel, restante_a_vender)
        if qtd_usada <= 1e-12:
            continue

        data_compra = datetime.strptime(lote["data_compra"], "%Y-%m-%d").date()
        dias_detido = (hoje - data_compra).days
        isento = dias_detido > 365

        preco_medio_compra = lote["valor_pago_eur"] / lote["quantidade"]
        custo_base = preco_medio_compra * qtd_usada

        ganho_desta_fatia = None
        if preco_venda_unitario is not None:
            valor_venda_fatia = preco_venda_unitario * qtd_usada
            ganho_desta_fatia = valor_venda_fatia - custo_base
            if isento:
                ganho_isento += ganho_desta_fatia
            else:
                ganho_tributavel += ganho_desta_fatia

        if isento:
            quantidade_isenta += qtd_usada
        else:
            quantidade_tributavel += qtd_usada

        custo_total_vendido += custo_base

        detalhe_lotes.append({
            "id_lote": lote["id"],
            "data_compra": lote["data_compra"],
            "plataforma": lote["plataforma"],
            "dias_detido": dias_detido,
            "isento": isento,
            "quantidade_usada": qtd_usada,
            "preco_medio_compra": preco_medio_compra,
            "custo_base": custo_base,
            "ganho": ganho_desta_fatia,
        })

        restante_a_vender -= qtd_usada

    return {
        "quantidade_isenta": quantidade_isenta,
        "quantidade_tributavel": quantidade_tributavel,
        "ganho_isento": ganho_isento,
        "ganho_tributavel": ganho_tributavel,
        "custo_total_vendido": custo_total_vendido,
        "detalhe_lotes": detalhe_lotes,
        "cobertura_completa": restante_a_vender <= 1e-9,
        "quantidade_em_falta": max(0.0, restante_a_vender),
    }


def confirmar_venda(ativo, quantidade_vender, preco_venda_unitario, plataforma_venda=""):
    """Aplica uma venda: abate a quantidade dos lotes por ordem FIFO e
    regista o histórico em st.session_state.vendas. Devolve None se não
    houver quantidade suficiente registada para vender."""
    resultado = obter_lotes_fifo(ativo, quantidade_vender, preco_venda_unitario)
    if not resultado["cobertura_completa"]:
        return None

    for fatia in resultado["detalhe_lotes"]:
        for compra in st.session_state.compras:
            if compra["id"] == fatia["id_lote"]:
                atual = compra.get("quantidade_restante", compra["quantidade"])
                compra["quantidade_restante"] = max(0.0, round(atual - fatia["quantidade_usada"], 10))
                break

    imposto_estimado = max(0.0, resultado["ganho_tributavel"]) * 0.28
    ...
    venda_registada = {
        ...
        "valor_venda_total": preco_venda_unitario * quantidade_vender,
        "plataforma": plataforma_venda,
        "ganho_isento": resultado["ganho_isento"],
        "ganho_tributavel": resultado["ganho_tributavel"],
        "imposto_estimado": imposto_estimado,
        "lotes_afetados": resultado["detalhe_lotes"],
    }
    st.session_state.vendas.append(venda_registada)
    return venda_registada
...
/* ---------- Cartão de métrica reutilizável (indicador néon à esquerda) ---------- */
.kc-metric-card {
    background: rgba(13, 17, 23, 0.72);
...
.kc-metric-card:hover {
    transform: translateY(-3px);
    box-shadow: 0 0 20px rgba(34, 211, 238, 0.18), 0 14px 30px rgba(0,0,0,0.5);
}
.kc-metric-card.green { border-left-color: #34D399; }
.kc-metric-card.orange { border-left-color: #FDBA74; }
.kc-metric-card.vermelho { border-left-color: #FF7C8E; }
.kc-metric-value.vermelho { color: #FF7C8E; }
...
/* ---------- Foco néon em inputs, selects e text areas ---------- */
...
div[data-testid="stDateInput"] input:focus {
    ...
    box-shadow: 0 0 0 1px #00E1FF, 0 0 14px rgba(0, 225, 255, 0.35) !important;
}
...
/* ---------- Glassmorphism nas caixas de aviso (info/warning/error/success) ---------- */
...
def html_cartao_lote(lote):
    ...
    parcialmente_vendido = lote["quantidade_restante"] < lote["quantidade"] - 1e-9
    if parcialmente_vendido:
        texto_quantidade = f"{lote['quantidade_restante']:.8f} restante (de {lote['quantidade']:.8f})"
    else:
        texto_quantidade = f"{lote['quantidade']:.8f}"
    ...
    <div class="cc-card {pulso_classe}" data-start="{lote['momento_compra'].isoformat()}" data-end="{lote['momento_isencao'].isoformat()}">
        ...
        <div class="cc-sub">{texto_quantidade} · {lote['plataforma']} · {formatar_euros(lote['valor_investido_restante'])}</div>
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("### Relatório de IRS 📄")
    st.caption(
        "Gera uma tabela pré-formatada para te ajudar a preencher o "
        "Anexo J (Quadro 9.2A / Quadro 25) da Modelo 3 do IRS."
    )

    if not st.session_state.vendas:
        st.caption("Ainda não registaste nenhuma venda nesta sessão.")
    else:
        linhas_relatorio = []
        for venda in st.session_state.vendas:
            for fatia in venda["lotes_afetados"]:
                linhas_relatorio.append({
                    "Código (G10/G11)": "G10" if fatia["isento"] else "G11",
                    "Ativo": venda["ativo"],
                    "Data de Aquisição": fatia["data_compra"],
                    "Data de Realização (Venda)": venda["data_venda"],
                    "Quantidade": round(fatia["quantidade_usada"], 8),
                    "Valor de Aquisição (€)": round(fatia["custo_base"], 2),
                    "Valor de Realização (€)": round(fatia["quantidade_usada"] * venda["preco_venda_unitario"], 2),
                    "Mais-Valia (€)": round(fatia["ganho"], 2) if fatia["ganho"] is not None else 0.0,
                    "Estado": "Isento (>365 dias)" if fatia["isento"] else "Tributável (28%)",
                })

        df_relatorio = pd.DataFrame(linhas_relatorio)
        st.dataframe(df_relatorio, use_container_width=True, hide_index=True)

        csv_relatorio = df_relatorio.to_csv(index=False, sep=";", decimal=",").encode("utf-8-sig")
        st.download_button(
            label="⬇️ Gerar Relatório de IRS (.csv)",
            data=csv_relatorio,
            file_name="cryptoclock_anexo_j.csv",
            mime="text/csv",
            use_container_width=True,
        )
        st.caption(
            "⚠️ Ferramenta de apoio, não uma submissão oficial — confirma sempre os "
            "valores com um contabilista antes de submeteres a declaração."
        )
...
for compra in st.session_state.compras:
    quantidade_restante = compra.get("quantidade_restante", compra["quantidade"])

    # lote totalmente vendido: já não aparece no painel nem nos totais
    if quantidade_restante <= 1e-9:
        continue

    preco_medio_compra = compra["valor_pago_eur"] / compra["quantidade"]
    valor_investido_restante = preco_medio_compra * quantidade_restante
    ...
    total_investido += valor_investido_restante
    if isento:
        total_isento += valor_investido_restante
    else:
        total_em_contagem += valor_investido_restante

    lotes_calculados.append({
        **compra,
        "quantidade_restante": quantidade_restante,
        "valor_investido_restante": valor_investido_restante,
        "momento_compra": momento_compra, "momento_isencao": momento_isencao,
...
# ABAS PRINCIPAIS
# ---------------------------------------------------------
tab_meu, tab_mercado = st.tabs([
    "🕒 O Meu CryptoClock", "📈 Cockpit de Mercado"
])
...
with tab_meu:
    st.markdown("""
    <div class="kc-lei-card">
        <div class="kc-lei-titulo">365 dias. É esse o número que decide tudo.</div>
        <div class="kc-lei-texto">
            Desde 2023, a lei portuguesa (Artigo 10.º do Código do IRS, Lei n.º 24-D/2022)
            estabelece uma regra simples mas implacável: mais-valias de criptoativos vendidos
            antes de completarem 365 dias de detenção são tributadas a 28%. Quem espera até ao
            dia 366... paga zero.<br><br>
            Não é uma lacuna. É a lei. E com a diretiva europeia DAC8 a entrar em vigor em 2026,
            obrigando as exchanges a reportar automaticamente cada transação à Autoridade
            Tributária, a única forma sensata de nunca seres apanhado de surpresa é saberes,
            ao segundo, exatamente quando cada lote teu se torna isento.
        </div>
        <div class="kc-lei-disclaimer">
            Este conteúdo tem fins informativos gerais e não substitui aconselhamento fiscal profissional.
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with kpi1:
        renderizar_cartao_metrica("💼", "Total Investido", formatar_euros(total_investido))
    with kpi2:
        renderizar_cartao_metrica("✅", "Ativos Isentos", formatar_euros(total_isento), cor_classe="green")
    with kpi3:
        renderizar_cartao_metrica("⏳", "Em Contagem", formatar_euros(total_em_contagem), cor_classe="orange")
    ...
    # -------------------------------------------------------
    # SIMULAR OU REGISTAR VENDA (FIFO + cálculo de imposto)
    # -------------------------------------------------------
    st.markdown('<div class="kc-section-title">💸 Simular ou Registar Venda</div>', unsafe_allow_html=True)

    ativos_disponiveis = sorted({
        c["ativo"] for c in st.session_state.compras
        if c.get("quantidade_restante", c["quantidade"]) > 1e-9
    })

    if not ativos_disponiveis:
        st.info("Assim que tiveres lotes ativos, podes simular uma venda aqui.")
    else:
        col_v1, col_v2, col_v3 = st.columns(3)
        with col_v1:
            ativo_venda = st.selectbox("Ativo", options=ativos_disponiveis, key="venda_ativo")
        with col_v2:
            quantidade_venda = st.number_input(
                "Quantidade a Vender", min_value=0.0, value=0.0, step=0.0001,
                format="%.8f", key="venda_quantidade",
            )
        with col_v3:
            preco_venda = st.number_input(
                "Preço de Venda por unidade (€)", min_value=0.0, value=0.0, step=0.01,
                format="%.2f", key="venda_preco",
            )

        if quantidade_venda > 0 and preco_venda > 0:
            simulacao = obter_lotes_fifo(ativo_venda, quantidade_venda, preco_venda)

            if not simulacao["cobertura_completa"]:
                st.error(
                    f"Só tens {quantidade_venda - simulacao['quantidade_em_falta']:.8f} {ativo_venda} "
                    f"disponíveis em lotes ativos — faltam {simulacao['quantidade_em_falta']:.8f}."
                )
            else:
                imposto_estimado = max(0.0, simulacao["ganho_tributavel"]) * 0.28
                ganho_total = simulacao["ganho_isento"] + simulacao["ganho_tributavel"]

                st.markdown(f"""
                <div class="kc-glass-card" style="margin-bottom:1rem;">
                    <div class="kc-metric-label">Desta venda de {quantidade_venda:.8f} {ativo_venda}</div>
                    <div class="kc-lei-texto" style="margin-top:6px;">
                        🟢 <b>{simulacao['quantidade_isenta']:.8f} {ativo_venda}</b> vem de lotes isentos (&gt;365 dias)<br>
                        ⚡ <b>{simulacao['quantidade_tributavel']:.8f} {ativo_venda}</b> vem de lotes ainda em contagem (&lt;365 dias)
                    </div>
                </div>
                """, unsafe_allow_html=True)
                ...
                with res1:
                    renderizar_cartao_metrica("📊", "Mais-Valia Total", formatar_euros(ganho_total))
                with res2:
                    renderizar_cartao_metrica(
                        "⚡", "Ganho Tributável", formatar_euros(simulacao['ganho_tributavel']),
                        cor_classe="orange",
                    )
                with res3:
                    renderizar_cartao_metrica(
                        "💰", "Imposto Estimado (28%)", formatar_euros(imposto_estimado),
                        cor_classe="vermelho",
                    )

                with st.expander("Ver detalhe por lote (FIFO)"):
                    for fatia in simulacao["detalhe_lotes"]:
                        estado_txt = "🟢 Isento" if fatia["isento"] else "⚡ Tributável"
                        st.markdown(
                            f"- **{fatia['quantidade_usada']:.8f}** de {fatia['data_compra']} "
                            f"({fatia['plataforma']}, {fatia['dias_detido']} dias detido) — {estado_txt}"
                            + (f" · ganho: {formatar_euros(fatia['ganho'])}" if fatia["ganho"] is not None else "")
                        )

                st.caption(
                    "⚠️ Este é apenas um cálculo estimado com base na regra dos 365 dias e na taxa "
                    "de 28% — não substitui aconselhamento fiscal profissional."
                )

                if st.button("✅ Confirmar Venda", key="botao_confirmar_venda"):
                    venda_registada = confirmar_venda(ativo_venda, quantidade_venda, preco_venda)
                    if venda_registada is None:
                        st.error("Não foi possível confirmar a venda — verifica a quantidade disponível.")
                    else:
                        st.success(
                            f"✅ Venda registada! Imposto estimado: "
                            f"{formatar_euros(venda_registada['imposto_estimado'])}"
                        )
                        st.rerun()
    ...
    if not lotes_calculados:
        ...
        with st.expander("🗑️ Gerir / eliminar um lote"):
            opcoes = {
                lote["id"]: f"{lote['ativo']} · {lote['plataforma']} · {lote['data_compra']} · {formatar_euros(lote['valor_investido_restante'])}"
                for lote in lotes_ordenados
            }
...
# ===========================================================
# CHAT FLUTUANTE — FÓRUM ANÓNIMO (popup no canto do ecrã)
# ===========================================================
...
with st.container(key="chat_toggle_wrap"):
    rotulo_botao = "✕" if st.session_state.chat_aberto else "💬"
    if st.button(rotulo_botao, key="botao_toggle_chat"):
        st.session_state.chat_aberto = not st.session_state.chat_aberto
        st.rerun()

if st.session_state.chat_aberto:
    with st.container(key="chat_painel_wrap"):
        st.markdown('<div class="kc-section-title">💬 Fórum Anónimo</div>', unsafe_allow_html=True)
        st.caption("Previsões de mercado e opiniões, de forma anónima e ao vivo.")
