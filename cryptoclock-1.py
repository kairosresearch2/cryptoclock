    ...
    if not ativos_disponiveis:
        ...
        if quantidade_venda > 0 and preco_venda > 0:
            ...
            if not simulacao["cobertura_completa"]:
                ...
                with res1:
                    st.markdown(f"""<div class="kc-glass-card"><div class="kc-metric-label">Mais-Valia Total</div>
                    <div class="kc-metric-value">{formatar_euros(ganho_total)}</div></div>""", unsafe_allow_html=True)
                with res2:
                    st.markdown(f"""<div class="kc-glass-card"><div class="kc-metric-label">Ganho Tributável</div>
                    <div class="kc-metric-value orange">{formatar_euros(simulacao['ganho_tributavel'])}</div></div>""", unsafe_allow_html=True)
                with res3:
                    st.markdown(f"""<div class="kc-glass-card"><div class="kc-metric-label">💰 Imposto Estimado (28%)</div>
                    <div class="kc-metric-value" style="color:#FF7C8E;">{formatar_euros(imposto_estimado)}</div></div>""", unsafe_allow_html=True)
